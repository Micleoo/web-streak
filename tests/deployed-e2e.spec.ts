...
test.describe('Testing Deployed Web Streak App (https://web-streak.vercel.app)', () => {
  test('Full E2E Flow: Register, Login, Complete Quest (XP & Bara check), Leaderboard, Profile', async ({ page }) => {
    ...
    await page.goto('https://web-streak.vercel.app/', { waitUntil: 'networkidle' });
    ...
    await expect(page.locator('.profile-name, h2')).toContainText(testName);
    const profileXp = page.locator('.stat-card').filter({ hasText: 'Total XP' }).locator('.stat-value');
    ...
    console.log(`[TEST] Semua pengujian (1, 2, 3) BERHASIL dengan sukses!`);
  });
});
